/**
 * @widget Calendar 日期组件，在页面上输出日期控件
 * 
 * @param {Object} params
 * @param {dom|selector|$} params.el 会在el里输出日期控件
...
define('ui/widget/Calendar/Calendar', function(require){
    var Controller = require('core/Controller'),
    	$ = require('lib/zepto');
    
    require('{theme}/TeaUI/Calendar/calendar.css');
    ...
    var Calendar = Controller.extend({
    	defaults: {
    		/**
    	...
    	initialize: function(){
    		var opts = this.attributes(),
    			eventHandler = this.proxy(this._eventHandler, this);
            ...
            opts.swipeable && this.on('swipeLeft swipeRight', eventHandler);
    	},
        ...
        _eventHandler: function(e) {
            ...
            switch (e.type) {
                ...
                case 'change':
                    elems = this.find('.ui-calendar-header .ui-calendar-year, ' +
                        '.ui-calendar-header .ui-calendar-month');
                ...
                default:
                    ...
                    if ((match = $(target).closest('.ui-calendar-calendar tbody a', root)) && match.length) {
                        ...
                        this.trigger('select', [date, this.formatDate(date), this]);
                        this.refresh();
        ...
        _option: function(key, val) {
            var opts = this.attributes(),
                date, minDate, maxDate, year, month, day;
            ...
            if (val !== undefined) {
                ...
                switch (key) {
                    case 'minDate':
                    case 'maxDate':
                        this.set(key, val ? this.parseDate(val) : null);
                        break;
                    ...
                    case 'date':
                        ...
                        this.set(key, this._option('selectedDate'));
                        break;
        ...
        select: function(dateStr){
        	...
        	this._option('selectedDate', date);
        ...
        refresh: function() {
            ...
            this.find('.ui-calendar-header select').off('change', eventHandler);
            this.html(this._generateHTML());
            this.find('.ui-calendar-header select').on('change', eventHandler);
            this.set('_invalid', false);
            return this;
        },
         ...
         * @grammar this.parseDate( str ) ⇒ Date
         */
