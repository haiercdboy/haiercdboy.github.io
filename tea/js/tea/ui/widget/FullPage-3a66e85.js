/**
 * @widget FullPage 全屏切换组件
 * 
 * @param {Object} params
 * @param {dom|selector|$} params.el el的children默认作为要切换的元素, 页面切换适应el的大小，而不是window
 * @param {Number} [params.width] 指定el的宽度
 * @param {Number} [params.height] 指定el的高度
 * @param {Number|String} [params.autoPlay] 为数字时表示自动播放指定次数，'loop'表示循环播放
 * @param {Number} [params.playInterval=5000] 自动播放的间隔时间 毫秒计算
 * @param {Number} [params.slideTime=800] 每页切换动画时长 毫秒计算
 * @param {String|Array} [params.easing='ease'] 'ease','ease-in', 'ease-out', 'ease-in-out', 'linear' or use cubic-bezier like [.33, 1.81, 1, 1];
 * @param {Object} [params.effect] 效果参数
 * @param {Array} [params.effect.opacity] 透明度 [opacityfrom, opacityto] 表示 [起始透明度, 结束时透明度]
 * @param {Object} [params.effect.transform]
 * @param {String} [params.effect.transform.translate] 切换方向 'X'|'Y'|'XY'|'none' 表示 X轴|Y轴|XY轴|无
 * @param {Array} [params.effect.transform.scale] 缩放 [scalefrom, scaleto] 表示 [起始缩放比例, 结束时缩放比例]
 * @param {Array} [params.effect.transform.rotate] 旋转 [rotatefrom, rotateto] 表示 [起始旋转角度, 结束时旋转角度]
 * @param {Boolean} [params.continuous=false] 是否循环(即能从最后页跳到第一页面)
 * @param {String} [params.mode=''] 转换模式 'wheel,touch,nav:navBar' 表示 '滚轮,触摸,导航条:导航条id'
 * @param {Function} [params.onSwipeStart] 触摸开始时的回调函数，当 retrun 'stop' 时,此次触摸将不会生效
 * @param {Function} [params.beforeChange] 滑动开始时的回调函数，当 retrun 'stop' 时,此次滑动将还原
 * @param {Function} [params.callback] 滑动结束后的回调函数
 * 
 * @Methods
 * prev() 直接滑向上一页
 * next() 直接滑入下一页
 ...
 * 
 * @Others
 * page绑定data-step="3",表示当前页面需要分3步执行，分别会在page上加上class .full-page1~3
 * data-animated="self"表示当前元素或其子元素不需要自动加上动画，需绑定在el、el.children()或需要动画的元素上
 * .animated-step1~6 先后执行
 * .animated.infinite 重复动画
 * 
 * @Source
 ...
 * 2014.4.2
 * 
 * 贝塞尔曲线动效 http://cubic-bezier.com/#.42,0,.58,1
 * @seanxphuang 修改点
 * 增加el、autoPlay、playInterval,width,height参数 增加data-animated="self", .animated-step1~6，.animated.infinite
 * 增加事件：swipeStart、beforeChange、change
 * 除了el、其他参数都是可选项
 * el及navbar可以是任意选择符
 * 页面切换适应el的大小，而不是window
 * 内置navBar样式 .full-navbar
 * 
 * @example
 * new FullPage({
 * 		autoPlay: 1,
...
define('ui/widget/FullPage', function(require){
	var isNumber = require('util/isNumber'),
		$ = require('lib/zepto');
	
	require('{theme}/TeaUI/FullPage/fullpage.css');
	
	//内置动画库
	var animations = [
		'bounce', 'flash', 'pulse', 'rubberBand', 'shake', 'swing', 'tada', 'wobble', 'jello', 'bounceIn',
		'bounceInDown', 'bounceInLeft', 'bounceInRight', 'bounceInUp', 'zoomInUp', 'rollIn',
		'fadeIn', 'fadeInDown', 'fadeInDownBig', 'flip', 'flipInX', 'flipInY', 'lightSpeedIn',
		'rotateIn', 'rotateInDownLeft', 'rotateInDownRight', 'rotateInUpLeft', 'rotateInUpRight',
		'slideInUp', 'slideInDown', 'slideInLeft', 'slideInRight', 'zoomIn', 'zoomInDown', 'zoomInLeft', 'zoomInRight'
	];
	
	var getRandom = function(){
		var max = animations.length - 1,
			random = Math.random();
...
function FullPage(options) {
	...
	var pageContain = $(options.el).addClass('full-pages')[0],//for Tea
		page = $(pageContain).children().addClass('full-page').get(),
		pagelen = page.length,
		iPage = pagelen,
		...
		currentClass = 'full-current', //for Tea
		swipingClass = 'full-swipe',
		slidingClass = 'full-slide',
		activeClass = 'full-active',
		stepClass = 'full-step';
	...
	init = function() {
		...
		pageRange = {
			X : options.width || $(pageContain).width() || $(window).width(), //for Tea
			Y : options.height || $(pageContain).height() || $(window).height()
		}

		pageContain.style.width = pageRange.X + 'px';
		pageContain.style.height = pageRange.Y + 'px';
		$(pageContain).children().first().addClass('full-current');
		
		//for Tea
		//自动给animated增加动画效果
		$(pageContain).data('animated') !== 'self' && $(page).each(function(){
			//如果有翻页嵌套时，外层不执行绑定animate，在子级初始化时再绑定
			if($(this).data('animated') === 'self' || $(this).find('.full-page').length) {
				return true;
			}
			
			$(this).find('.animated').each(function(){
				if($(this).data('animated') === 'self') {
					return true;
				}
				
				$(this).addClass(animations[getRandom()]);
			});
	...
	if (_isNav) {
		navChange = function(from, to) {
			...
			replaceClass(navChildren[from], activeClass, SPACE);
			navChildren[to].className = t === '' ? activeClass : t + ' ' + activeClass;
		}
	}
	...
	checkStep = function(direct) {
		...
		if (newStep >= 0 && newStep <= stepArr[indexNow]) {
			if (newStep === 0) {
				replaceClass(page[indexNow], stepClass + '1', '');
				stepNow[indexNow] = newStep;
				return false;
			}
			if (newStep === 1 && oldStep === 0) {
				page[indexNow].className += ' ' + stepClass + '1';
				stepNow[indexNow] = newStep;
				return false;
			}
			replaceClass(page[indexNow], stepClass + oldStep, stepClass + newStep);
			stepNow[indexNow] = newStep;
			return false;
		}
		return true;
	...
	goPage = function(to) {
		...
		setTimeout(function() {
			...
			page[to].className += ' ' + slidingClass;	
		}, fix);
		...
		setTimeout(function() {
			...
			replaceClass(page[indexOld], currentClass, '');
			replaceClass(page[indexNow], slidingClass, currentClass);

			if (options.callback) {
				options.callback(indexNow, page[indexNow]);
				$(pageContain).trigger('change', [indexNow, page[indexNow]]);
			}
	...
	page[indexNow].className = _t.indexOf(currentClass) !== -1 ? _t : _t + ' ' + currentClass;
		...
		(function(m) {
			...
			switch (true) {
				...
				(function() {
					...
					if (effect.transform.translate === 'Y') {
						setIndex = function() {
							...
							if (prev) {
								...
								page[pr].className += ' ' + swipingClass ;
							}
							if (next) {
								...
								page[ne].className += ' ' + swipingClass;
							}
						}
						move = function (o) {
						...
						validReset = function(s, n) {
							...
							s.opacity = 1;
							replaceClass(page[to], swipingClass, slidingClass);
							s[browser.cssCore + 'TransitionDuration'] = _t + 'ms';
							s[browser.cssCore + 'Transform'] = 'translate(0,0) translateZ(0)';
							setTimeout(function() {
								replaceClass(page[indexNow], currentClass, '');
								replaceClass(page[to], slidingClass, currentClass);
								indexNow = to;
								if (options.callback) {
									options.callback(indexNow, page[indexNow]);
								}
					...
					touchEvent = {
						start : function(e) {
							...
							//for Tea
							$(pageContain).trigger('swipeStart');
						...
						move : function(e) {
							...
							//for Tea
								...
								$(pageContain).trigger('beforeChange');
				...
				(function() {
					...
					var navId = m.split('nav:')[1], //for Tea
						navObj = $(navId)[0],
						navLen,
						gotoPage,
						_t;
					
					//effect.transform.translate === 'Y' && $(navObj).addClass('full-navbar-vertical');
					...
					if (_t.indexOf(activeClass) === -1) {
						navChildren[indexNow].className = _t === '' ? activeClass : _t + ' ' + activeClass;
					}
...
var autoPlay = function(to, interval) {
	var me = this;
    me.interval && clearTimeout(me.interval);
...
var adapter = function(params){
	var params = $.extend(true, {}, defaults, params),
		fullPage = new FullPage(params),
		isAutoPlay = params.autoPlay;
	
	if(isAutoPlay) {
		isNumber(isAutoPlay) && (fullPage.playNum = 1);
		
		fullPage.interval = setTimeout(function() {
	    	fullPage.go(fullPage.thisPage() + 1);
		}, params.playInterval || 5000);
		
		var callback = params.callback;
		params.callback = function(index, thisPage){
			if(isNumber(params.autoPlay) || params.autoPlay === 'loop') {
				index = index + 1 >= fullPage.pageLen ? 0 : index + 1;
				
				if(fullPage.playNum && index === 0) {
					++fullPage.playNum;
					
					if(fullPage.playNum > isAutoPlay) {
						params.autoPlay = false;
						return callback && callback.apply(this, arguments);
					}
				}
				
		        autoPlay.call(fullPage, index, params.playInterval);
	        }
